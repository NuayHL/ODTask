    ...
    for idx, imgid in enumerate(imgs):
        ...
        with open(outputpath + "/" + img["file_name"]+".txt", "a") as f:
            for ann in anns:
                ...
                ann["bbox"][3] = ann["bbox"][3] if ann["bbox"][1] + ann["bbox"][3] <= w \
                                else h - ann["bbox"][1]
                ...
                f.write(bboxsys)
        progressbar(idx/float(lenth), barlenth = 40)

if __name__ == "__main__":
    coco2txt("CrowdHuman/annotation_train_coco_style.json", "label/Images_train")
    coco2txt("CrowdHuman/annotation_val_coco_style.json", "label/Images_val")